...
class Hooks():
    ...
    async def milk(self, worker, channel, cow_name):
        ...
        if self.ranch.is_milking_channel(channel):
            ...
            if worker.lower() != cow_name.lower() and is_cow and is_worker and is_online:
                data = await self.ranch.logic.milk_cow(worker, cow_name)  # (success, amount, lvlup, milk)
                
                print (data)
                message = ""
                
                if data[0] == True:
                    message = f"[user]{worker}[/user] milked [user]{cow_name}[/user] and got {data[1]}l of milk!"
    ...
    async def remove_cow(self, user, cow_name):
        """
        Disable a cow
        ...
        """
        if self.ranch.client.is_priviliged(user.strip()):
            status = self.ranch.logic.disable_cow(cow_name)
            if status:
                message = f"cow [user]{cow_name}[/user] was disabled!"
                
            else:
                message = f"could not disable cow [user]{cow_name}[/user]!"
