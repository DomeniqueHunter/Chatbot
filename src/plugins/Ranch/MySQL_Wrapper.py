...
class RANCH_DB(DB_WRAPPER):
    ...
    def milk_cow(self, cow_name, worker_name, amount, date):
        statement = f"""
                INSERT INTO milking(worker_id, cow_id, amount, date) VALUES(
    ...
    def check_milking(self, cow_name, worker_name, date):
        statement = f"""
                SELECT COUNT(*)
                FROM milking
                WHERE worker_id = (SELECT id FROM worker WHERE person_id = (SELECT id FROM person WHERE lower(name) = lower('{worker_name}')))
                AND cow_id = (SELECT id FROM cow WHERE person_id = (SELECT id FROM person WHERE lower(name) = lower('{cow_name}')))
