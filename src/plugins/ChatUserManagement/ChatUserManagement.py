...
class ChatUserManagement(Plugin_Prototype):
    ...
    async def choose_tool(self, user, input_string = "list all 1"):
        if (self.client.is_owner(user)):
            ...
            if tool == 'list':
                ...
                await self.client.send_private_message("{} is no valid tool!".format(tool), user)
    ...
    async def tool_find(self, user, name):
        try:
            ...
            await self.client.send_private_message("[user]{}[/user] : {}".format(character['identity'], character['gender']), user)
        except:
            await self.client.send_private_message("{} does not exists".format(name), user)
    ...
    async def gender_stats(self, user):
        if (self.client.is_owner(user)):
            ...
            for gender in genders:     
                ...
                msg += "{}: {}% - {}\n".format(gender, round(amount/sall*100,2), amount)
